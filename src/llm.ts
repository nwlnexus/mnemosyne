...
async function defaultRunClaude(prompt: string): Promise<string> {
  const { stdout } = await execFileAsync("claude", ["-p", prompt], { maxBuffer: 10 * 1024 * 1024 });
  return stdout;
}

export async function callLLM(prompt: string, deps: LLMDeps = {}): Promise<string> {
  const fetchImpl = deps.fetchImpl ?? fetch;
  const runClaude = deps.runClaude ?? defaultRunClaude;
  const ollamaUrl = deps.ollamaUrl ?? process.env.MNEMOSYNE_OLLAMA_URL ?? "http://ai-hub.raptor-mimosa.ts.net:11434";
